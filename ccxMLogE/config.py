...
def f_mdConfproPath(userPath, confname):
    ...
    if os.path.exists(conf_path):
        ...
        if os.path.exists(pathConf):
            ...
            # 写日志，返回错误码 ，二期
            print('错误码002 找不到%s超参数配置文件' % confname)
            pass
    else:
        # 写日志，返回错误码 ，二期
...
def f_mdAllconf(userPath):
    confnameLs = ['ccxboost_demo.conf', 'ccxboost_speed.conf', 'ccxboost_accuracy.conf', 'ccxboost_stable.conf',
                  ...
                  'ccxboost_bayes_demo.conf', 'ccxboost_bayes_speed.conf', 'ccxboost_bayes_accuracy.conf',
                  'ccxboost_bayes_stable.conf',
                  'ccxgbm_bayes_demo.conf', 'ccxgbm_bayes_speed.conf', 'ccxgbm_bayes_accuracy.conf',
                  'ccxgbm_bayes_stable.conf',
                  'ccxrf_bayes_demo.conf', 'ccxrf_bayes_speed.conf', 'ccxrf_bayes_accuracy.conf',
                  'ccxrf_bayes_stable.conf'
                  ]
    ls = []
...
#
# 发现这么做的一个不好处 有问题
# LOGFILEPATH = r'C:\Users\liyin\Desktop\ccxml\CcxMLOGE-0.2.2\TestUnit\LOG'  # 指定路径
# LOGFILEPATH = r'C:\Users\liyin\Desktop\CcxMLOGE'  # 指定路径
LOGFILEPATH = r'/ccxml/liyingkun/CcxMLOGE-0.2.2/TestUnit/LOG'  # 指定路径
...
# modelurl = 'http://10.0.5.136:9999/output/api'  # 开发环境请求接口
